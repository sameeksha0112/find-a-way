...
const FireServiceCard = ({ fireserviceMap }) => {
  const r = fireserviceMap.location;
        ...
        <div class=" card h-100 ">
          <div class="d-flex flex-column justify-content-between m-0 p-0 card-body">
            <div class="card-header bg-warning">
              <h5 class="card-title">{fireserviceMap.name}</h5>
            </div>
            <p class="card-text">
              <address> <span className="fw-bold">Address: </span>{fireserviceMap.address}</address>
            </p>
            <div>
              <p className="fw-bold bg-danger d-inline-block rounded p-2 text-white">
                Contract: {fireserviceMap.phone}
              </p>
            </div>
...
export default FireServiceCard;
