...
const PoliceStationCard = ({ policestationMap }) => {
  const r = policestationMap.location;
        ...
        <div class=" card h-100 ">
          <div class="d-flex flex-column justify-content-between m-0 p-0 card-body">
            <div class="card-header bg-warning">
              <h5 class="card-title">{policestationMap.name}</h5>
            </div>
            <p class="card-text p-2">
              <address><span className="fw-bold">Address :</span>{policestationMap.address}</address>
            </p>
            <div>
              <p className="fw-bold bg-danger d-inline-block rounded p-2 text-white">
                Contract: {policestationMap.phone}
              </p>
            </div>
